...
%% veg
veg_names = {'simulation_number', 'year', 'DoY', ...
    'GPP', 'Ci/Ca', 'WUE', 'Electron_transport', 'NPQ_energy', 'NPQ_photon', ...
    'canopy_level_FQE','LST','emis'};
veg_units = {'', '', '', ...
    'umol m-2 s-1', '', 'g(C) kg-1(H2O)', 'umol m-2 s-1', 'W m-2', '',...
    'umol photons (umol photons)-1', 'K',''};
write_output(veg_names, veg_units, fnames.veg_file, n_col.veg, ns)

%% flu
flu_names = {'simulation_number','nu_iterations', 'mean error sunlit', 'year','DoY',...
    'Rnctot','lEctot','Hctot','Actot','Tcave', ...
    'Rnstot','lEstot','Hstot','Gstot','Tsave','Rnus','lEus','Hus','Gu'...
    'Rntot','lEtot','Htot','Gtot'};
flu_units = {'', '', 'W m-2', '', '',  ...
    'W m-2','W m-2','W m-2','umol m-2 s-1','C',...
    'W m-2','W m-2','W m-2','W m-2','C','W m-2','W m-2','W m-2','W m-2',...
...
if isfield(fnames, 'fluor_file')
    fluor_names = {'F_1stpeak', 'wl_1stpeak', 'F_2ndpeak', 'wl_2ndpeak', 'F684', 'F758', 'LFtot', 'EFtot', 'EFtot_RC'};
...
%% resistances
write_output({'aerodynamic resistance (raa)', 'ra for soil', 'rss',...
    'ustar','RiB','Ta surf','Ga', 'Gcw', 'decoupling factor'}, ...
    {'s m-1','s m-1','s m-1','m s-1','','C','m s-1','m s-1',''}, ...
...
%     dlmwrite(n_csv, out_2d, '-append', 'precision', '%d'); % SLOW!
    for k=1:ns
        fprintf(f_csv, '%f,', out_2d(k, 1:end-1));
